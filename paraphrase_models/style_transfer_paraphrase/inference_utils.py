...
class GPT2Generator(object):
    def __init__(self, model_path, upper_length="eos", beam_size=1, top_p=0.0, device='cuda'):
    ...
    def generate_batch(self, contexts, global_dense_features=None, get_scores=False,
        ...
        output, _, scores = self.gpt2_model.generate(
            gpt2_sentences=torch.tensor([inst.sentence for inst in instances]).to(self.device),
            segments=torch.tensor([inst.segment for inst in instances]).to(self.device),
            global_dense_vectors=torch.tensor([inst.gdv for inst in instances]).to(self.device),
